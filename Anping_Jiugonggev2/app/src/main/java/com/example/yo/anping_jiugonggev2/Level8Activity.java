...
public class Level8Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        FloatingActionButton floatingActionButton = (FloatingActionButton) findViewById(R.id.fab8);
        ...
        Button button = (Button) findViewById(R.id.solvebt8);
        sharedPreferences = getSharedPreferences("Level8Preferences",MODE_PRIVATE);
        isPreferencesSolve = sharedPreferences.getBoolean("PreferencesSolve",false);
        if(isPreferencesSolve){
            textView = (TextView) findViewById(R.id.level_tv8);
            textView.setText("風獅爺");
        }
    ...
    private boolean getUserLocation() {
        ...
        if(gps.canGetLocation){
            //關卡經緯度
            double level_8_latitude = 23.0007085;
            double level_8_longitude = 120.1616817;
            // 使用者經緯度
            double user_latitude = gps.getLatitude();
            double user_longitude = gps.getLongitude();
            //兩者距離
            double distance = getDistance(level_8_latitude,level_8_longitude,user_latitude,user_longitude);
