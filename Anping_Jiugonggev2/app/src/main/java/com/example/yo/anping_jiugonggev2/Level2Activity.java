...
public class Level2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button button = (Button) findViewById(R.id.solvebt2);
        sharedPreferences = getSharedPreferences("Level2Preferences",MODE_PRIVATE);
        isPreferencesSolve = sharedPreferences.getBoolean("PreferencesSolve",false);
        if(isPreferencesSolve){
            textView = (TextView) findViewById(R.id.level_tv2);
            textView.setText("海山館劍獅");
        }
    ...
    private boolean getUserLocation() {
        ...
        if(gps.canGetLocation){
            //關卡經緯度
            double level_2_latitude = 23.0012376;
            double level_2_longitude = 120.1627642;
            // 使用者經緯度
            double user_latitude = gps.getLatitude();
            double user_longitude = gps.getLongitude();
            //兩者距離
            double distance = getDistance(level_2_latitude,level_2_longitude,user_latitude,user_longitude);
