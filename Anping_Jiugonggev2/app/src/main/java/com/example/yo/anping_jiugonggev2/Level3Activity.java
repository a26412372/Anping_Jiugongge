...
public class Level3Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button button = (Button) findViewById(R.id.solvebt3);
        sharedPreferences = getSharedPreferences("Level3Preferences",MODE_PRIVATE);
        isPreferencesSolve = sharedPreferences.getBoolean("PreferencesSolve",false);
        if(isPreferencesSolve){
            textView = (TextView) findViewById(R.id.level_tv3);
            textView.setText("有擋頭劍獅");
        }
    ...
    private boolean getUserLocation() {
        ...
        if(gps.canGetLocation){
            //關卡經緯度
            double level_3_latitude = 23.0023117;
            double level_3_longitude = 120.1620425;
            // 使用者經緯度
            double user_latitude = gps.getLatitude();
            double user_longitude = gps.getLongitude();
            //兩者距離
            double distance = getDistance(level_3_latitude,level_3_longitude,user_latitude,user_longitude);
